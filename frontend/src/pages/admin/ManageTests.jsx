...
export default function ManageTests() {
  ...
  const fetchAll = () => {
    Promise.all([
      fetch("http://localhost:8080/api/admin/tests", { headers }).then(r => r.json()),
      fetch("http://localhost:8080/api/admin/subjects", { headers }).then(r => r.json()),
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const url = editingTest ? `http://localhost:8080/api/admin/tests/${editingTest.id}` : "http://localhost:8080/api/admin/tests";
  ...
  const toggleActive = async (id) => {
    await fetch(`http://localhost:8080/api/admin/tests/${id}/toggle`, { method: "PUT", headers });
    fetchAll();
  };

  const handleDelete = async (id) => {
    await fetch(`http://localhost:8080/api/admin/tests/${id}`, { method: "DELETE", headers });
    fetchAll();
