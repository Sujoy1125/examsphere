...
export default function QuestionManagement() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`http://localhost:8080/api/tests/${selectedTest}/questions`, { headers })
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const url = editingQ ? `http://localhost:8080/api/admin/questions/${editingQ.id}` : "http://localhost:8080/api/admin/questions";
      const method = editingQ ? "PUT" : "POST";
      await fetch(url, { method, headers, body: JSON.stringify({ ...form, testId: Number(form.testId) }) });
      // Reload questions
      const res = await fetch(`http://localhost:8080/api/tests/${selectedTest}/questions`, { headers });
  ...
  const handleDelete = async (id) => {
    await fetch(`http://localhost:8080/api/admin/questions/${id}`, { method: "DELETE", headers });
